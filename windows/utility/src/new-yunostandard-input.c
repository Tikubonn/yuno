...
yunofile *__stdcall __new_yunostandard_input (bool asyncp, yunosize buffersize, yunomemory **globalmemoryp){
	reset_yunoerror();
	...
	if (open_yunostandard_input(asyncp, buffer, buffersize, file) != 0){
		yunoerror lasterror = get_yunoerror();
		__free_yunomemory(file, sizeof(yunofile), globalmemoryp);
